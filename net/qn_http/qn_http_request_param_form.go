...
// It returns <def> if <key> does not exist in the form and <def> is given, or else it returns nil.
func (r *Request) GetFormVar(key string, def ...interface{}) qn_var.Var {
	return qn_var.New(r.GetForm(key, def...))
}
...
// GetFormMapStrVar retrieves and returns all form parameters passed from client as map[string]qn_var.Var.
// The parameter <kvMap> specifies the keys retrieving from client parameters, the associated values
// are the default values if the client does not pass.
func (r *Request) GetFormMapStrVar(kvMap ...map[string]interface{}) map[string]qn_var.Var {
	postMap := r.GetFormMap(kvMap...)
	if len(postMap) > 0 {
		m := make(map[string]qn_var.Var, len(postMap))
		for k, v := range postMap {
			m[k] = qn_var.New(v)
		}
		return m
