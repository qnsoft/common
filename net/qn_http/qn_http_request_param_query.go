...
func (r *Request) GetQueryVar(key string, def ...interface{}) qn_var.Var {
	return qn_var.New(r.GetQuery(key, def...))
}
...
// as map[string]qn_var.Var. The parameter <kvMap> specifies the keys
// retrieving from client parameters, the associated values are the default values if the client
// does not pass.
func (r *Request) GetQueryMapStrVar(kvMap ...map[string]interface{}) map[string]qn_var.Var {
	queryMap := r.GetQueryMap(kvMap...)
	if len(queryMap) > 0 {
		m := make(map[string]qn_var.Var, len(queryMap))
		for k, v := range queryMap {
			m[k] = qn_var.New(v)
		}
		return m
