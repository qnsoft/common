...
// custom params as qn_var.Var, no matter what HTTP method the client is using. The parameter
// <def> specifies the default value if the <key> does not exist.
func (r *Request) GetRequestVar(key string, def ...interface{}) qn_var.Var {
	return qn_var.New(r.GetRequest(key, def...))
}
...
// params as map[string]qn_var.Var, no matter what HTTP method the client is using. The parameter
// <kvMap> specifies the keys retrieving from client parameters, the associated values are the
// default values if the client does not pass.
func (r *Request) GetRequestMapStrVar(kvMap ...map[string]interface{}) map[string]qn_var.Var {
	requestMap := r.GetRequestMap(kvMap...)
	if len(requestMap) > 0 {
		m := make(map[string]qn_var.Var, len(requestMap))
		for k, v := range requestMap {
			m[k] = qn_var.New(v)
		}
		return m
