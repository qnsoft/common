...
// qn_log.Println(s.SendMail("notify@a.com", "ulric@b.com;rain@c.com", "subject", "body, <font color=red>red</font>"))
package qn_smtp
...
func (s *SMTP) SendMail(from, tos, subject, body string, contentType ...string) error {
	...
	header := map[string]string{
		...
		"Subject":                   fmt.Sprintf("=?UTF-8?B?%s?=", contentEncoding.EncodeToString(qn_conv.UnsafeStrToBytes(subject))),
		"MIME-Version":              "1.0",
	...
	message += "\r\n" + contentEncoding.EncodeToString(qn_conv.UnsafeStrToBytes(body))
	return smtp.SendMail(
		address,
		smtp.PlainAuth("", s.Username, s.Password, server),
		from,
		tosArr,
		qn_conv.UnsafeStrToBytes(message),
	)
}
