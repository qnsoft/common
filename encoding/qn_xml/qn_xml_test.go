...
import (
	...
	"github.com/qnsoft/common/encoding/qn_charset"
...
func buildXml(charset string, str string) (string, string) {
	...
	srcXml, err = qn_charset.UTF8To(charset, srcXml)
	if err != nil {
		return "", ""
	}
...
func Test_XmlToJson(t *testing.T) {
	for _, v := range testData {
		...
		srcJson, err := qn_xml.ToJson([]byte(srcXml))
		if err != nil {
			t.Errorf("qn_xml.ToJson error. %s", srcXml)
		}

		dstJson, err := qn_xml.ToJson([]byte(dstXml))
		if err != nil {
...
func Test_Decode1(t *testing.T) {
	for _, v := range testData {
		...
		srcMap, err := qn_xml.Decode([]byte(srcXml))
		if err != nil {
			t.Errorf("qn_xml.Decode error. %s", srcXml)
		}

		dstMap, err := qn_xml.Decode([]byte(dstXml))
		if err != nil {
			t.Errorf("qn_xml decode error. %s", dstXml)
		}
...
`
		m, err := qn_xml.Decode([]byte(content))
		t.Assert(err, nil)
...
<?xml version="1.0" encoding="UTF-8"?><doc><username>johngcn</username><password1>123456</password1><password2>123456</password2></doc>
`
		m, err := qn_xml.DecodeWithoutRoot([]byte(content))
		t.Assert(err, nil)
...
func Test_EncodeIndent(t *testing.T) {
	...
	_, err := qn_xml.EncodeWithIndent(m, "xml")
	if err != nil {
...
func TestErrXml(t *testing.T) {
	for _, v := range testErrData {
		...
		_, err := qn_xml.ToJson([]byte(srcXml))
		if err == nil {
...
func TestErrCase(t *testing.T) {
	qn_test.C(t, func(t *qn_test.T) {
		...
		_, err := qn_xml.ToJson([]byte(errXml))
		if err == nil {
	...
	qn_test.C(t, func(t *qn_test.T) {
		errXml := `<root><bool>true</bool><float>100.92</float><int>123</int><string>hello world</string>`
		_, err := qn_xml.Decode([]byte(errXml))
		if err == nil {
			t.Errorf("unexpected value: nil")
		}
	})
