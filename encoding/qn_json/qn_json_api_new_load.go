...
func doLoadContent(dataType string, data []byte, safe ...bool) (*Json, error) {
	...
	case "xml", ".xml":
		if data, err = qn_xml.ToJson(data); err != nil {
			return nil, err
		}

	case "yml", "yaml", ".yml", ".yaml":
		if data, err = qn_yaml.ToJson(data); err != nil {
			return nil, err
		}

	case "toml", ".toml":
		if data, err = qn_toml.ToJson(data); err != nil {
			return nil, err
		}
	case "ini", ".ini":
		if data, err = qn_int.ToJson(data); err != nil {
			return nil, err
		}
	default:
