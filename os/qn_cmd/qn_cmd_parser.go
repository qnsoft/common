...
func (p *Parser) GetOptVar(name string, def ...interface{}) qn_var.Var {
	if p.ContainsOpt(name) {
		return qn_var.New(p.GetOpt(name))
	}
	if len(def) > 0 {
		return qn_var.New(def[0])
	}
	return qn_var.New(nil)
}
...
func (p *Parser) GetArgVar(index int, def ...string) qn_var.Var {
	return qn_var.New(p.GetArg(index, def...))
}
